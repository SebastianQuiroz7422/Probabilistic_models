...
class RTRBM(object):
    def __init__(self, data: torch.Tensor, n_hidden: int = 10, device: str = 'cpu', debug_mode: bool = False):
        ...
        self.W = 0.01/self.n_visible * torch.randn(self.n_hidden, self.n_visible, dtype=torch.float, device=self.device)
    ...
    def _parallel_recurrent_sample_r_given_v(self, v: torch.Tensor) -> torch.Tensor:
        '''
        Sample the expected recurrent hidden units given the visible units in parallel over batches.
    ...
    def _parallel_sample_r_h_given_v(self, v: torch.Tensor, r: torch.Tensor) -> torch.Tensor:
        '''
        Sample the expected hidden- and hidden units given the visible units in parallel over batches.
    ...
    def _parallel_sample_v_given_h(self, h: torch.Tensor) -> torch.Tensor:
        '''
        Sample the visible units given the hidden units in parallel over batches.
    ...
    def grad(self, v_data: torch.Tensor, r_data: torch.Tensor, ht_k: torch.Tensor, vt_k: torch.Tensor, barvt: torch.Tensor, barht: torch.Tensor) -> torch.Tensor:
        ...
        Computes the gradient of the loss function with respect to the parameters of the model.
...
if __name__ == '__main__':
    ...
    from data.reshape_data import reshape, train_test_split
    ...
    n_h = 3
    norm = 0.35
    delay = 1 # interaction time delay
    ...
    U = torch.tensor([
        [0, -1, 1],
        [1, 0, -1],
        [-1, 1, 0]], dtype=torch.float) / norm

    s = PoissonTimeShiftedData(
        neurons_per_population=20,
        n_populations=n_h, n_batches=35,
        time_steps_per_batch=1000,
        fr_mode='gaussian', delay=delay, temporal_connections=U, norm=1,
        frequency_range=frequency_range, amplitude_range=amplitude_range, phase_range=phase_range,
        std_gaus=[0.001, 0.002])

    axes = s.plot_stats(T=100)
    plt.show()

    data = reshape(s.data)
    train, test = train_test_split(data, train_batches=80, test_batches=20)

    rtrbm = RTRBM(train, n_hidden=n_h, device='cuda')
    rtrbm.learn(batch_size=50, n_epochs=500, lr_schedule='geometric_decay',
                max_lr=5e-3, min_lr=1e-4, CDk=10, mom=0.7, wc=0, sp=0, x=0)
    ...
    vt_infer, rt_infer = rtrbm.infer(torch.tensor(test[:, :50, 0]), t_extra=50)

    # effective coupling
    W = rtrbm.W.detach().clone().to('cpu').numpy()
    U = rtrbm.U.detach().clone().to('cpu').numpy()
    rt = np.array(rtrbm._parallel_recurrent_sample_r_given_v(test.to(rtrbm.device)).to('cpu'))
    var_h_matrix = np.reshape(np.var(rt[..., 0], 1).repeat(W.shape[1]), [W.shape[1], W.shape[0]]).T
    var_v_matrix = np.reshape(np.var(train[..., 0].detach().numpy(), 1).repeat(W.shape[0]), [W.shape[0], W.shape[1]])
    ...
    sns.heatmap(vt_infer.to('cpu'), ax=ax[0, 0], cbar=False)
